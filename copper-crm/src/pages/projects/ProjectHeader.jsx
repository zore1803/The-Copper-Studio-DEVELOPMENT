            ...
            <div className="min-w-0">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-1.5 text-xs text-[#6b7280]">
                <span className="flex items-center gap-1.5">
                  <Calendar size={13} className="text-[#C55418]" /> Start: <span className="font-semibold text-[#111827]">{startDate}</span>
                </span>
                <span className="flex items-center gap-1.5">
                  <CalendarCheck size={13} className="text-[#C55418]" /> Expected: <span className="font-semibold text-[#111827]">{expectedDate}</span>
                </span>
                {packageName && (
                  <span className="flex items-center gap-1.5">
                    <Package size={13} className="text-[#C55418]" /> Package: <span className="font-semibold text-[#111827]">{packageName}</span>
                  </span>
                )}
              </div>
            </div>
          ...
          {tabs.map((tab) => (
            <Link
              ...
              className={`whitespace-nowrap rounded-full px-4 py-2 text-sm font-bold transition-colors ${
                tab.label === activeTab ? "bg-[#C55418] text-white" : "text-[#6b7280] hover:bg-[#f9fafb]"
              }`}
            >
              {tab.label}
            </Link>
