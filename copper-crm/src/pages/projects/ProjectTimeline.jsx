...
function KanbanView({ tasks, project, onDragEnd, onOpenNew, onOpenEdit }) {
  const columns = useMemo(() => {
    ...
    if (project && Array.isArray(project.stages)) {
      project.stages.forEach((stage, idx) => {
        ...
        if (!grouped[mappedStatus]) mappedStatus = "Backlog";

        grouped[mappedStatus].push({
          isStage: true,
          projectId: String(project.id || project._id || "unknown"),
          stageIndex: idx,
          id: `stage-${project.id || project._id}-${idx}`,
...
export default function ProjectTimeline() {
  ...
  async function handleDragEnd(columns, result) {
    ...
    if (movedTask.isStage) {
      ...
      if (project) {
        const updatedProj = { ...project, stages: [...(project.stages || [])] };
  ...
  return (
    <div className="space-y-6">
      ...
      {view === "kanban" ? (
        <KanbanView tasks={projectTasks} project={project} onDragEnd={handleDragEnd} onOpenNew={openNewTask} onOpenEdit={openEditTask} />
      ) : (
