...
function Field({ label, value, onChange, type = "text", options, disabled = false, hint = "" }) {
  return (
    <label className="block">
      ...
      {options ? (
        <select value={value || ""} onChange={(e) => onChange(e.target.value)} disabled={disabled} className="mt-1.5 w-full rounded-lg border border-[#e5e7eb] px-3 py-2 text-sm outline-none focus:border-[#5A1A14] disabled:bg-[#FFFFFF] disabled:text-[#6b7280]">
          {options.map((opt) => (
            typeof opt === "string"
        ...
        <input type={type} value={value || ""} onChange={(e) => onChange(e.target.value)} disabled={disabled} className="mt-1.5 w-full rounded-lg border border-[#e5e7eb] px-3 py-2 text-sm outline-none focus:border-[#5A1A14] focus:ring-2 focus:ring-[#5A1A14]/20 disabled:bg-[#FFFFFF] disabled:text-[#6b7280]" />
      )}
      ...
      <div className="relative mb-5">
        ...
        <div className="relative mt-1.5">
          ...
          <input
            ...
            className="w-full rounded-lg border border-[#e5e7eb] pl-9 pr-3 py-2 text-sm outline-none focus:border-[#5A1A14] focus:ring-2 focus:ring-[#5A1A14]/20"
          />
        ...
        <div className="flex flex-col gap-3 border-b border-[#FFFFFF] px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-wrap gap-2">
            {["All", ...INVOICE_STATUSES].map((item) => (
              <button key={item} onClick={() => setStatus(item)} className={`rounded-lg px-3 py-1.5 text-xs font-semibold ${status === item ? "bg-[#5A1A14] text-white" : "bg-[#FFFFFF] text-[#6b7280]"}`}>{item}</button>
            ))}
          </div>
              ...
              <tbody className="divide-y divide-[#FFFFFF]">
                {filtered.map((invoice) => (
                  <tr key={invoice._id || invoice.id || invoice.invoiceNumber} className="hover:bg-[#E5E7EB]">
                    ...
                    <td className="px-4 py-3"><button onClick={() => downloadInvoice(invoice)} className="text-[#5A1A14] hover:underline"><Download size={15} /></button></td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function StatusSelect({ value, onChange }) {
  return (
    <select
      ...
      className={`rounded-full border-0 px-2 py-1 text-xs font-semibold outline-none ring-1 ring-transparent transition focus:ring-[#5A1A14]/30 ${statusTone(value)}`}
