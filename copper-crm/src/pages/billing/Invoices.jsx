...
function Field({ label, value, onChange, type = "text", options, disabled = false, hint = "" }) {
  return (
    <label className="block">
      ...
      {options ? (
        <select value={value || ""} onChange={(e) => onChange(e.target.value)} disabled={disabled} className="mt-1.5 w-full rounded-lg border border-[#e5e7eb] px-3 py-2 text-sm outline-none focus:border-[#8D3118] disabled:bg-[#FFFFFF] disabled:text-[#6b7280]">
          {options.map((opt) => (
            typeof opt === "string"
        ...
        <input type={type} value={value || ""} onChange={(e) => onChange(e.target.value)} disabled={disabled} className="mt-1.5 w-full rounded-lg border border-[#e5e7eb] px-3 py-2 text-sm outline-none focus:border-[#8D3118] focus:ring-2 focus:ring-[#8D3118]/20 disabled:bg-[#FFFFFF] disabled:text-[#6b7280]" />
      )}
        ...
        <div className="flex flex-col gap-3 border-b border-[#FFFFFF] px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-wrap gap-2">
            {["All", ...INVOICE_STATUSES].map((item) => (
              <button key={item} onClick={() => setStatus(item)} className={`rounded-lg px-3 py-1.5 text-xs font-semibold ${status === item ? "bg-[#8D3118] text-white" : "bg-[#FFFFFF] text-[#6b7280]"}`}>{item}</button>
            ))}
          </div>
