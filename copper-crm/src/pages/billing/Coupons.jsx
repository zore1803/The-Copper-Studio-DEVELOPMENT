      ...
      >
        ...
        {items.map((item) => (
          <div
            ...
            className={`flex cursor-pointer items-center justify-center text-sm font-semibold transition-colors select-none ${item === selected ? "text-[#8D3118]" : "text-[#6B7280]"}`}
          >
            {item}
          </div>
        ))}
  ...
  return (
    <div className="absolute right-0 bottom-full z-50 mb-1 w-52 overflow-hidden rounded-xl border border-[#e5e7eb] bg-white shadow-2xl shadow-black/15">
      ...
      <div className="bg-[#8D3118] px-3 py-2 text-center">
        <p className="text-[9px] font-bold uppercase tracking-widest text-[#8D3118]">Select Time</p>
        <p className="text-2xl font-bold text-white tracking-wide">
          {h12str}<span className="text-[#8D3118]">:</span>{minStr}
          <span className="ml-1.5 text-base font-semibold text-[#8D3118]">{merStr}</span>
        </p>
      </div>
    ...
    <div ref={wrapRef} className="relative">
      ...
      <div className={`mt-1.5 flex items-center rounded-lg border transition-all focus-within:ring-2 ${error ? "border-red-300 focus-within:ring-red-100" : "border-[#e5e7eb] focus-within:border-[#8D3118] focus-within:ring-[#8D3118]/20"}`}>
        ...
        <button
          ...
          className={`-ml-1 mr-2 flex h-7 w-7 shrink-0 items-center justify-center rounded-md transition-colors ${clockOpen ? "text-[#8D3118]" : "text-[#6B7280] hover:text-[#8D3118]"}`}
        >
          <Clock size={14} />
        </button>
      </div>
...
function CouponFormPanel({ onClose, onCreate }) {
  ...
  return (
    ...
    >
      {/* Preview */}
      <div className="mb-4 rounded-xl border border-dashed border-[#8D3118] bg-[#FFFFFF] p-4 text-center">
  ...
  return (
    <div className="rounded-xl border border-[#e5e7eb] bg-[#ffffff] p-4">
      <div className="flex items-center gap-3">
        <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#FFFFFF] text-[#8D3118]"><Icon size={17} /></div>
        <div>
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => onCopy(coupon.code)} className="shrink-0 text-[#6B7280] hover:text-[#8D3118]"><Copy size={13} /></button>
    ...
    <div className="flex items-center gap-3 border-b border-[#FFFFFF] px-4 py-3 last:border-0 hover:bg-[#E5E7EB]">
      <div className="min-w-0 flex-1 grid grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,0.8fr)] gap-3 items-center text-sm">
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => onCopy(coupon.code)} className="shrink-0 text-[#6B7280] hover:text-[#8D3118]"><Copy size={12} /></button>
          ...
          <div className="flex flex-col gap-3 border-b border-[#FFFFFF] px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center rounded-lg border border-[#e5e7eb] overflow-hidden">
                <button onClick={() => setViewMode("card")} className={`flex h-9 w-9 items-center justify-center transition-colors ${viewMode === "card" ? "bg-[#8D3118] text-white" : "bg-white text-[#6b7280] hover:bg-[#E5E7EB]"}`}><Grid2x2 size={15} /></button>
                <button onClick={() => setViewMode("list")} className={`flex h-9 w-9 items-center justify-center transition-colors ${viewMode === "list" ? "bg-[#8D3118] text-white" : "bg-white text-[#6b7280] hover:bg-[#E5E7EB]"}`}><List size={15} /></button>
              </div>
            </div>
          </div>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1).slice(0, 5).map((p) => (
                      <button
                        key={p}
                        onClick={() => setPage(p)}
                        className={`flex h-8 w-8 items-center justify-center rounded-lg text-sm font-semibold transition-colors ${p === page ? "bg-[#8D3118] text-white" : "border border-[#e5e7eb] bg-white text-[#1A1A1A] hover:bg-[#E5E7EB]"}`}
                      >
                        {p}
                      </button>
                    ))}
