...
function WebsiteTextLink({ href }) {
  ...
  return (
    <a
      ...
      rel="noopener noreferrer"
      title={href}
      className="inline-flex items-center gap-1.5 rounded-full border border-[#d8c2b9] bg-white px-2.5 py-1 text-xs font-semibold text-[#C55418] shadow-sm transition-colors hover:bg-[#fff8f6]"
    >
...
function EmptyState({ icon: Icon, title, text, action }) {
  return (
    <div className="rounded-xl border border-dashed border-[#E1E4EA] bg-white p-10 text-center">
      <div className="mx-auto mb-3 flex h-11 w-11 items-center justify-center rounded-lg bg-[#fff1ec] text-[#C55418]">
        <Icon size={20} />
      </div>
        ...
        {projects.length ? (
          projects.map((project) => {
            ...
            return (
              <label
                key={id}
                className={`flex items-center justify-between rounded-lg border px-3 py-2 text-sm cursor-pointer transition-colors ${checked ? "border-[#C55418] bg-[#fff1ec]" : "border-[#e5e7eb] hover:bg-[#f9fafb]"}`}
              >
                <span className="font-semibold text-[#111827]">{project.name}</span>
                <input type="checkbox" checked={checked} onChange={() => toggleProject(id)} className="h-4 w-4 rounded border-[#d1d5db] accent-[#C55418]" />
              </label>
            );
          })
        ) : (
          ...
          <div className="mt-5 rounded-xl border border-[#FFFFFF] bg-[#fafafa]">
            <div className="grid grid-cols-2 gap-4 p-4 sm:grid-cols-3 lg:grid-cols-[repeat(5,minmax(0,1fr))_auto]">
              ...
              <button
                ...
                className="flex h-9 w-9 items-center justify-center self-center justify-self-end rounded-full border border-[#e5d3cc] bg-white text-[#C55418] transition-colors hover:bg-[#fff1ec] sm:col-start-3 lg:col-start-auto"
              >
          ...
          <div className="inline-flex items-center gap-1 rounded-full border border-[#e5e7eb] bg-white p-1">
            {TABS.map((tab) => (
              <button
                ...
                className={`flex items-center gap-1.5 whitespace-nowrap rounded-full px-4 py-2 text-sm font-bold transition-colors ${activeTab === tab ? "bg-[#C55418] text-white" : "text-[#6b7280] hover:bg-[#f9fafb]"}`}
              >
                {tab}
          ...
          {filteredItems.map((item, index) => {
            ...
            return (
              <div key={`${item.type}-${item.title}-${index}`} className="flex gap-3">
                <div className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-[#fff1ec] text-[#C55418]">
                  <Icon size={15} />
                </div>
                <div className="min-w-0">
    ...
    <div className="inline-flex h-11 items-center rounded-full border border-[#e5e7eb] bg-white p-1">
      {options.map((option) => (
        <button
          ...
          className={`rounded-full px-3 py-1.5 text-xs font-bold ${value === option ? "bg-[#C55418] text-white" : "text-[#6b7280] hover:bg-[#f9fafb]"}`}
        >
          {option}
        </button>
      ))}
        ...
        <tbody className="divide-y divide-[#f3f4f6] bg-white">
          {contacts.map((contact) => {
            ...
            <tr key={contact.id || contact._id}>
              ...
              <td className="py-3 pr-4 text-[#374151]">{contact.linkedin ? <a className="text-[#C55418] hover:underline" href={contact.linkedin} target="_blank" rel="noreferrer">Open</a> : "No LinkedIn"}</td>
              <td className="py-3 pr-4"><StatusBadge status={contact.status || "Active"} /></td>
              <td className="py-3 pr-5 text-right">
                <div className="inline-flex items-center gap-2">
                  <button onClick={() => onView(contact)} className="rounded-lg p-2 text-[#6b7280] hover:bg-[#f3f4f6] hover:text-[#C55418]"><Eye size={14} /></button>
                  <button onClick={() => onEdit(contact)} className="rounded-lg p-2 text-[#6b7280] hover:bg-[#f3f4f6] hover:text-[#C55418]"><Edit2 size={14} /></button>
        ...
        <tbody className="divide-y divide-[#f3f4f6] bg-white">
          {invoices.map((invoice) => (
            <tr key={invoice.id || invoice._id}>
              ...
              <td className="py-3 pr-5 text-right">
                <div className="inline-flex items-center gap-2">
                  <button onClick={() => onView(invoice)} className="rounded-lg p-2 text-[#6b7280] hover:bg-[#f3f4f6] hover:text-[#C55418]"><Eye size={14} /></button>
                  <button onClick={() => onDownload(invoice)} className="rounded-lg p-2 text-[#6b7280] hover:bg-[#f3f4f6] hover:text-[#C55418]"><Download size={14} /></button>
                </div>
              </td>
            </tr>
          ))}
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center rounded-full border border-[#e5e7eb] bg-white p-0.5">
            <button
              ...
              className={`rounded-full p-1.5 ${view === "Grid" ? "bg-[#fff1ec] text-[#C55418]" : "text-[#9ca3af] hover:text-[#374151]"}`}
              title="Grid view"
            >
              <LayoutGrid size={15} />
            </button>
            <button
              ...
              className={`rounded-full p-1.5 ${view === "List" ? "bg-[#fff1ec] text-[#C55418]" : "text-[#9ca3af] hover:text-[#374151]"}`}
              title="List view"
            >
              <ListIcon size={15} />
            </button>
        ...
        return (
          <div key={doc.id || doc._id || name} className="flex items-center justify-between gap-3 rounded-xl border border-[#e5e7eb] bg-white p-3">
            ...
            <div className="flex shrink-0 items-center gap-2">
              {canOpen ? (
                <a href={doc.fileUrl} target="_blank" rel="noreferrer" className="rounded-lg bg-[#C55418] px-3 py-1.5 text-xs font-semibold text-white hover:bg-[#9A4113]">
                  View
                </a>
              ) : (
          ...
          <div className="grid grid-cols-7">
            {monthCells.map((cell, index) => {
              ...
              return (
                ...
                >
                  <span className={`flex h-5 w-5 items-center justify-center rounded-full text-[11px] font-bold ${isToday ? "bg-[#C55418] text-white" : "text-[#374151]"}`}>
                    {cell.date.getDate()}
                  </span>
                  {cell.tasks.length > 0 && (
          ...
          {weekDays.map(({ date, tasks: dayTasksList }) => {
            ...
            return (
              ...
              >
                ...
                <span className={`mt-1 flex h-6 w-6 items-center justify-center rounded-full text-xs font-bold ${isToday ? "bg-[#C55418] text-white" : "text-[#374151]"}`}>{date.getDate()}</span>
                <div className="mt-2 space-y-1">
                  {dayTasksList.slice(0, 3).map((task) => (
                    <p key={task.id || task._id} className="truncate rounded bg-[#fff1ec] px-1.5 py-1 text-[10px] font-semibold text-[#C55418]">{task.title || task.taskName || "Task"}</p>
                  ))}
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          className={`flex h-9 items-center gap-1.5 rounded-lg border px-2.5 text-xs font-semibold transition-colors ${
            sortDir ? "border-[#C55418] bg-[#fff8f6] text-[#C55418]" : "border-[#e5e7eb] text-[#6b7280] hover:bg-[#f9fafb]"
          }`}
        ...
        {hasFilters && (
          <button
            ...
            className="rounded-lg px-2 py-1.5 text-xs font-semibold text-[#C55418] hover:bg-[#fff1ec]"
          >
            Clear
          </button>
        )}
            ...
            >
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex shrink-0 items-center gap-1">
                  <button type="button" onClick={() => onEdit(note)} className="rounded-lg p-1.5 text-[#9ca3af] hover:bg-[#fff1ec] hover:text-[#C55418]" title="Edit note">
                    <Edit2 size={14} />
                  </button>
