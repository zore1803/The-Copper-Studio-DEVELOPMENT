  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button onClick={onAdd} className="flex h-[42px] items-center gap-1.5 self-start rounded-full bg-[#5A1A14] px-3.5 text-xs font-medium text-white transition-colors hover:bg-[#6B7280] sm:self-auto">
      ...
      {contacts.length === 0 ? (
        <div className="rounded-xl border border-dashed border-[#E5E7EB] bg-white py-12 text-center">
          ...
          <button onClick={onAdd} className="mt-3 inline-flex items-center gap-1.5 rounded-full border border-[#5A1A14] px-3.5 py-2 text-xs font-medium text-[#5A1A14] transition-colors hover:bg-[#E5E7EB]">
            ...
            return (
              <div key={c._id || c.id} className="group relative flex flex-col gap-2 rounded-xl border border-[#E5E7EB] bg-white p-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => onOpenContact(c)} className="inline-flex items-center gap-1 text-xs font-medium text-[#5A1A14] hover:underline">
                    <Eye size={13} /> Open
                  </button>
                </div>
              </div>
    ...
    >
      ...
      <div className="flex flex-col gap-1.5">
        {list.map((c) => {
          ...
          return (
            <label key={id} className={`flex cursor-pointer items-center gap-3 rounded-lg border px-3 py-2.5 transition-colors ${checked ? "border-[#5A1A14] bg-[#FFFFFF]" : "border-[#e5e7eb] hover:bg-[#E5E7EB]"}`}>
              <input type="checkbox" checked={checked} onChange={() => toggle(id)} className="rounded border-[#E5E7EB] accent-[#5A1A14]" />
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold text-[#1A1A1A]">{contactFullName(c)}</p>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-4 rounded-lg border border-[#E5E7EB] bg-white p-6 shadow-[0_4px_4px_rgba(0,0,0,0.05)] sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setCreatingFolder(true)} className="flex h-[42px] items-center gap-1.5 whitespace-nowrap rounded-full bg-[#5A1A14] px-3.5 text-xs font-medium text-white hover:bg-[#6B7280] transition-colors">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    {Array.from({ length: folderTotalPages }, (_, i) => i + 1).slice(0, 5).map((p) => (
                      <button
                        ...
                        className={`flex h-8 w-8 items-center justify-center rounded-lg text-sm font-semibold transition-colors ${p === folderPage ? "bg-[#5A1A14] text-white" : "border border-[#E5E7EB] text-[#6B7280] hover:bg-[#E5E7EB]"}`}
                      >
                        {p}
                      </button>
                    ))}
