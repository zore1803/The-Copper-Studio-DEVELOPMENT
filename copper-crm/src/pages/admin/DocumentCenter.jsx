        ...
        <div className="flex flex-wrap items-center gap-2">
          {mode !== "root" && (
            <select
              ...
              className="h-9 rounded-lg border border-[#e5e7eb] bg-white px-3 text-sm text-[#374151] outline-none focus:border-[#C55418] focus:ring-2 focus:ring-[#C55418]/20"
            >
            ...
            <div className="rounded-xl border border-[#e5e7eb] bg-white px-6 pt-5">
              <div className="flex items-start justify-between gap-4">
                ...
                {currentProject.projectId && (
                  <span className="shrink-0 rounded-full bg-[#fff1ec] px-3 py-1 text-xs font-bold text-[#C55418]">{currentProject.projectId}</span>
                )}
              </div>
              <div className="mt-4 flex gap-1">
                {[["details", "Project Details"], ["documents", `Documents (${displayFiles.length})`]].map(([key, label]) => (
                  <button
                    ...
                    className={`-mb-px border-b-2 px-4 py-2 text-sm font-semibold ${projectTab === key ? "border-[#C55418] text-[#C55418]" : "border-transparent text-[#6b7280] hover:text-[#111827]"}`}
                  >
                    {label}
                  </button>
                ))}
