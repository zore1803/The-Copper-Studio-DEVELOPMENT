        ...
        <div className="grid grid-cols-7">
          {cells.map((cell, index) => {
            ...
            return (
              ...
              >
                <span className={`flex h-5 w-5 items-center justify-center rounded-full text-[11px] font-bold ${isToday ? "bg-[#C55418] text-white" : "text-gray-700"}`}>
                  {cell.date.getDate()}
                </span>
                {cell.meetings.length > 0 && (
                  <div className="mt-1 flex flex-wrap gap-0.5">
                    {cell.meetings.slice(0, 3).map((m) => <span key={m.id || m._id} className="h-1.5 w-1.5 rounded-full bg-[#C55418]" />)}
                    {cell.meetings.length > 3 && <span className="text-[9px] font-bold text-[#C55418]">+{cell.meetings.length - 3}</span>}
                  </div>
                )}
              </button>
            );
    ...
    <div className="p-5 xl:p-6 bg-[#FFFFFF] min-h-full">
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {tab === "Tasks" && (
            <button onClick={() => setSelected({ title: "", relatedTo: "", assigned: "", due: "", status: "Backlog", priority: "Medium" })} className="flex h-9 items-center gap-1.5 rounded-lg bg-[#C55418] px-3 text-xs font-semibold text-white hover:bg-blue-600">
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-gray-100 px-5 py-3">
          <div className="flex gap-1">
            {["Tasks", "Meetings"].map((t) => (
              <button key={t} onClick={() => switchTab(t)} className={`rounded-full px-3 py-1.5 text-xs font-semibold transition-colors ${tab === t ? "border-b-2 border-[#C55418] text-[#C55418] rounded-none pb-1" : "text-gray-500 hover:text-gray-700"}`}>{t}</button>
            ))}
          </div>
          <div className="ml-auto flex items-center gap-2">
            ...
            <div className="relative">
              <button onClick={() => setFiltersOpen((v) => !v)} className={`flex h-8 w-8 items-center justify-center rounded-lg border transition-colors ${statusFilter !== "All" ? "border-[#C55418] bg-blue-50 text-[#C55418]" : "border-gray-200 text-gray-400 hover:bg-gray-50"}`}>
                ...
                <div className="absolute right-0 top-full z-20 mt-1 w-44 rounded-xl border border-gray-200 bg-white p-2 shadow-lg">
                  ...
                  {["All", ...statusOptions].map((status) => (
                    <button
                      ...
                      className={`flex w-full items-center rounded-lg px-2 py-1.5 text-left text-xs font-semibold capitalize ${statusFilter === status ? "bg-blue-50 text-[#C55418]" : "text-gray-600 hover:bg-gray-50"}`}
                    >
                      {status}
                    </button>
                  ))}
            ...
            <button onClick={() => setView("list")} className={`flex h-8 w-8 items-center justify-center rounded-lg border transition-colors ${view === "list" ? "border-[#C55418] bg-blue-50 text-[#C55418]" : "border-gray-200 text-gray-400 hover:bg-gray-50"}`}><List size={13} /></button>
            <button onClick={() => setView("kanban")} className={`flex h-8 w-8 items-center justify-center rounded-lg border transition-colors ${view === "kanban" ? "border-[#C55418] bg-blue-50 text-[#C55418]" : "border-gray-200 text-gray-400 hover:bg-gray-50"}`}><LayoutGrid size={13} /></button>
            {tab === "Meetings" && (
              <button onClick={() => setView("calendar")} className={`flex h-8 items-center gap-1.5 rounded-lg border px-2.5 text-xs font-semibold transition-colors ${view === "calendar" ? "border-[#C55418] bg-blue-50 text-[#C55418]" : "border-gray-200 text-gray-600 hover:bg-gray-50"}`}>
                          ...
                          {updatingId === mtg.id ? (
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              {mtg.status === "confirmed" && (
                                <button onClick={() => updateMeetingStatus(mtg, "completed")} className="rounded-lg bg-[#C55418] px-2.5 py-1 text-[11px] font-bold text-white hover:bg-blue-600">Mark Done</button>
                              )}
            ...
            <div className="flex items-center justify-between border-t border-gray-100 px-5 py-3">
              ...
              <div className="flex items-center gap-1">
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                  <button key={p} onClick={() => setPage(p)} className={`flex h-7 w-7 items-center justify-center rounded-lg border text-xs font-semibold ${p === page ? "border-[#C55418] bg-[#C55418] text-white" : "border-gray-200 text-gray-500 hover:bg-gray-50"}`}>{p}</button>
                ))}
  ...
  return (
    <label className="block">
      ...
      <select
        value={value}
        onChange={(event) => onChange(event.target.value)}
        className="mt-2 w-full rounded-2xl border border-[#6B7280] bg-[#FFFFFF] px-4 py-3 text-sm text-[#1A1A1A] outline-none transition-all focus:border-[#C55418] focus:ring-4 focus:ring-[#E5E7EB]"
      >
        {options.map((option) => (
  ...
  return (
    <div className="flex items-start justify-between gap-4 rounded-2xl border border-[#E5E7EB] bg-[#FFFFFF] px-4 py-4">
      ...
      <button
        ...
        className={`relative h-7 w-12 rounded-full transition-colors ${checked ? "bg-[#C55418]" : "bg-[#6B7280]"}`}
      >
        <span
      ...
      <div className="relative mt-2">
        <input
          ...
          className={`w-full rounded-2xl border bg-[#FFFFFF] px-4 py-3 pr-11 text-sm text-[#1A1A1A] outline-none transition-all focus:ring-4 ${
            error ? "border-red-300 focus:border-red-400 focus:ring-red-100" : "border-[#6B7280] focus:border-[#C55418] focus:ring-[#E5E7EB]"
          }`}
        />
        <button
          type="button"
          ...
          className="absolute right-3.5 top-1/2 -translate-y-1/2 text-[#6B7280] transition-colors hover:text-[#C55418]"
        >
...
function SecurityGate({ onUnlock }) {
  ...
  return (
    <div className="flex flex-col items-center px-6 py-14 text-center">
      <div className="grid h-14 w-14 place-items-center rounded-2xl bg-[#E5E7EB] text-[#C55418]"><LockKeyhole size={22} /></div>
      ...
      {sections.map((section) => (
        ...
        >
          <div className={`grid h-10 w-10 shrink-0 place-items-center rounded-2xl ${
            activeSection === section.key ? "bg-[#E5E7EB] text-[#C55418]" : "bg-[#E5E7EB] text-[#6B7280]"
          }`}>
    ...
    <div className="py-4 first:pt-0">
      ...
      <div className="mt-2.5 flex items-center gap-1.5">
        <input
          ...
          className="h-8 flex-1 rounded-lg border border-[#e5e7eb] bg-[#FFFFFF] px-3 text-xs text-[#1A1A1A] outline-none transition-all focus:border-[#C55418] focus:bg-white focus:ring-2 focus:ring-[#C55418]/20"
        />
        <button type="button" onClick={addValue} className="flex h-8 items-center gap-1 rounded-lg border border-[#e5e7eb] bg-white px-2.5 text-xs font-semibold text-[#1A1A1A] hover:border-[#C55418] hover:text-[#C55418] transition-colors">
          <Plus size={12} /> Add
        </button>
      </div>
    </div>
        ...
        {DATA_FIELD_GROUPS.map((group) => {
          ...
          return (
            <button
              ...
              className={`flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-sm transition-colors ${isActive ? "bg-[#FFFFFF] font-semibold text-[#C55418]" : "text-[#1A1A1A] hover:bg-[#E5E7EB]"}`}
            >
              <span>{group.label}</span>
              <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-bold ${isActive ? "bg-[#C55418]/10 text-[#C55418]" : "bg-[#FFFFFF] text-[#6B7280]"}`}>{totalOptions}</span>
            </button>
          );
        })}
      </nav>
    ...
    <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <linearGradient id="ig-body" x1="24" y1="28" x2="24" y2="46" gradientUnits="userSpaceOnUse">
          <stop stopColor="#C55418" />
          <stop offset="1" stopColor="#C55418" />
        </linearGradient>
      ...
      {/* eyes */}
      <circle cx="21" cy="17" r="1.2" fill="#C55418" />
      <circle cx="27" cy="17" r="1.2" fill="#C55418" />
      {/* smile */}
      <path d="M21 21.5q3 2.5 6 0" stroke="#C55418" strokeWidth="1.2" strokeLinecap="round" fill="none" />
      {/* badge / checkmark ring */}
      <circle cx="36" cy="12" r="5.5" fill="#C55418" />
...
function IconTemplates() {
  return (
    <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* envelope flap */}
      <path d="M5 16l14 10 14-10" stroke="#C55418" strokeWidth="1.5" fill="none" strokeLinecap="round" />
      {/* lines in email body */}
      <rect x="10" y="23" width="12" height="1.5" rx="0.75" fill="#C55418" opacity="0.4" />
      <rect x="10" y="26.5" width="8" height="1.5" rx="0.75" fill="#C55418" opacity="0.4" />
      {/* whatsapp bubble */}
...
function IconPricing() {
  return (
    <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <path d="M39 5l1 3 3 1-3 1-1 3-1-3-3-1 3-1z" fill="#C55418" />
    </svg>
  );
}
    ...
    <div className="flex h-full flex-col bg-[#FFFFFF]">
      <div className="flex flex-col gap-4 border-b border-[#E5E7EB] bg-white px-6 py-3 lg:h-14 lg:flex-row lg:items-center lg:justify-between lg:gap-4 lg:py-0">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[#E5E7EB] text-[#C55418]"><Icon size={17} /></div>
          <div className="min-w-0">
  ...
  return (
    <SettingsSubPage title="Trigger Template" description="Email and WhatsApp message templates." icon={MessageCircle}>
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        >
          <div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-[#E5E7EB] text-[#C55418]"><Mail size={18} /></div>
          <div>
        ...
        <button
          ...
          className="flex items-start gap-3 rounded-2xl border border-[#E5E7EB] bg-white p-5 text-left transition-colors hover:bg-[#E5E7EB]"
        >
          <div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-[#E5E7EB] text-[#C55418]"><MessageCircle size={18} /></div>
          <div>
...
function PricingPreviewCards({ visible, edits, activeCategory, onCategory }) {
  return (
    <div style={{ background: "#FFFFFF", padding: "8px 4px 4px", fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      {/* Floating pill category switcher \u2014 same as the live page */}
      {onCategory && (
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 22 }}>
          <div style={{ display: "inline-flex", gap: 4, padding: 5, background: "#FFFFFF", borderRadius: 16 }}>
          ...
          return (
            ...
            >
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
                ...
                <Circle size={20} style={{ color: "#C55418", flexShrink: 0, marginTop: 2 }} />
              </div>
  ...
  return (
    <div className="flex min-h-0 flex-1 gap-0 divide-x divide-[#FFFFFF]">
      ...
      <nav className="w-44 shrink-0 py-1 pr-4">
        {PRICING_CATEGORIES.map((cat) => {
          ...
          return (
            ...
            >
              <span>{cat}</span>
              <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-bold ${isActive ? "bg-[#C55418]/10 text-[#C55418]" : "bg-[#FFFFFF] text-[#6B7280]"}`}>{count}</span>
            </button>
          );
        })}
      </nav>
      ...
      <div className="flex min-h-0 flex-1 flex-col pl-6">
        ...
        {mode === "preview" ? (
          <div className="flex-1 overflow-y-auto rounded-xl border border-[#E5E7EB] bg-[#FFFFFF] p-5">
            ...
            {visible.map((pkg) => {
              ...
              return (
                <div key={pkg.id} className="rounded-xl border border-[#E5E7EB] bg-[#FFFFFF] p-5">
                  <p className="mb-4 text-xs font-bold uppercase tracking-wide text-[#C55418]">{pkg.name}</p>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div>
                      <label className="mb-1 block text-xs font-medium text-[#6B7280]">Plan label</label>
                      <input
                        className="h-9 w-full rounded-lg border border-[#e5e7eb] bg-white px-3 text-sm text-[#1A1A1A] focus:border-[#C55418] focus:outline-none focus:ring-2 focus:ring-[#C55418]/20"
                        value={e.label ?? ""}
                    ...
                    <div className="sm:col-span-2">
                      ...
                      <textarea
                        rows={5}
                        className="w-full rounded-lg border border-[#e5e7eb] bg-white px-3 py-2 text-sm text-[#1A1A1A] focus:border-[#C55418] focus:outline-none focus:ring-2 focus:ring-[#C55418]/20 resize-none"
                        value={e.includes ?? ""}
