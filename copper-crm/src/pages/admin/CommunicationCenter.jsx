...
function blocksToHtml(blocks) {
  return blocks.map((b) => {
    ...
    switch (b.type) {
      ...
      case "box":
        return `<div style="margin:16px 0;padding:14px 16px;border:1px solid #fde2d6;background:#fff8f6;border-radius:12px">${b.title ? `<p style="margin:0 0 6px;font-weight:700;color:#C55418">${b.title}</p>` : ""}<p style="margin:0;font-size:14px;color:#525866">${t}</p></div>`;
      case "divider":
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex rounded-lg border border-[#e5e7eb] p-0.5 text-[11px] font-semibold">
                  <button type="button" onClick={() => switchMode("text")}
                    className={`rounded-md px-2.5 py-1 transition-colors ${bodyMode === "text" ? "bg-[#C55418] text-white" : "text-[#6b7280] hover:text-[#374151]"}`}
                  >Text</button>
                  <button type="button" onClick={() => switchMode("json")}
                    className={`rounded-md px-2.5 py-1 transition-colors ${bodyMode === "json" ? "bg-[#C55418] text-white" : "text-[#6b7280] hover:text-[#374151]"}`}
                  >JSON</button>
                </div>
              </div>
              <textarea
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                {VARIABLES.map((v) => (
                  <button
                    ...
                    className="rounded bg-white border border-[#e5e7eb] px-1.5 py-0.5 font-mono text-[11px] text-[#374151] hover:border-[#C55418] hover:text-[#C55418]"
                  >{`{{${v}}}`}</button>
                ))}
              </div>
            </div>
          ...
          {defaults.map((def) => {
            ...
            return (
              ...
              >
                ...
                {added ? <span className="shrink-0 text-[10px] font-bold uppercase text-[#9ca3af]">Added</span> : <Plus size={14} className="shrink-0 text-[#C55418]" />}
              </button>
            );
          })}
        </div>
    ...
    <div className="flex min-h-full flex-col bg-[#FFFFFF]">
      ...
      <div className="flex flex-col gap-4 border-b border-[#E1E4EA] bg-white px-6 py-3 lg:h-14 lg:flex-row lg:items-center lg:justify-between lg:gap-4 lg:py-0">
        <div className="flex min-w-0 items-center gap-2.5">
          <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-[#fff1ec] text-[#C55418]">
            <PageIcon size={15} />
          </div>
