              ...
              {users.map((u, i) => (
                <div key={u._id || u.id || i} onClick={() => navigate(`/admin/companies/${u._id || u.id}`)} className="cursor-pointer p-3 border border-[#E1E4EA] rounded-lg hover:border-[#cda88f] hover:shadow-sm bg-[#ffffff] transition-all">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-[10px] font-bold text-[#9CA3AF] bg-[#FFFFFF] px-2 py-1 rounded">CLIENT</span>
                  </div>
                </div>
              ))}
            </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-[#EAECF0] px-5 py-4 gap-3">
                ...
                <div className="flex gap-2">
                  {["All", "Revenue", "Users", "Projects"].map(m => (
                    <button key={m} onClick={() => setMetricFilter(m)} className={`px-2 py-1 text-[11px] font-bold rounded transition-colors ${metricFilter === m ? 'bg-[#8D3118] text-white' : 'bg-[#FFFFFF] text-[#6B7280] hover:bg-gray-200'}`}>
                      {m}
                    </button>
                  ))}
                </div>
...
function PriorityProjectsTable({ projects, page, setPage, search, setSearch, navigate }) {
  ...
  const getBadgeColor = (level) => {
    switch (level) {
      ...
      case "Completed": return "bg-[#FFFFFF] text-[#374151] border-[#E1E4EA]";
      default: return "bg-[#FFFFFF] text-[#374151] border-[#E1E4EA]";
    }
  };
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm whitespace-nowrap">
          <thead className="bg-[#8D3118] border-b border-[#6E2412]">
            <tr>
              <th className="px-5 py-3 text-left text-xs font-medium text-white/90">Project Name</th>
              <th className="px-5 py-3 text-left text-xs font-medium text-white/90">Company / Contact</th>
              <th className="px-5 py-3 text-left text-xs font-medium text-white/90">Package</th>
              <th className="px-5 py-3 text-left text-xs font-medium text-white/90">Timeline</th>
              <th className="px-5 py-3 text-left text-xs font-medium text-white/90">Progress</th>
              <th className="px-5 py-3 text-left text-xs font-medium text-white/90">Priority</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100 bg-[#ffffff]">
            {displayed.length > 0 ? displayed.map(p => (
              ...
              >
                ...
                <td className="px-5 py-4">
                  <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium bg-[#FFFFFF] text-[#1F2937]">
...
function RecentPaymentsTable({ payments, page, setPage, search, setSearch, navigate }) {
  ...
  const getStatusColor = (status) => {
    ...
    if (s.includes("refund")) return "bg-[#FFFFFF] text-[#374151] border-[#E1E4EA]";
    return "bg-[#FFFFFF] text-[#374151] border-[#E1E4EA]";
  };
    ...
    <Card>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm whitespace-nowrap">
          <thead className="bg-[#8D3118] border-b border-[#6E2412]">
            <tr>
              <th className="px-5 py-3 text-left text-xs font-medium text-white/90">Date</th>
              <th className="px-5 py-3 text-left text-xs font-medium text-white/90">Company / Contact</th>
              <th className="px-5 py-3 text-left text-xs font-medium text-white/90">Package / Order ID</th>
              <th className="px-5 py-3 text-right text-xs font-medium text-white/90">Amount</th>
              <th className="px-5 py-3 text-left text-xs font-medium text-white/90">Status</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100 bg-[#ffffff]">
            {displayed.length > 0 ? displayed.map(p => (
