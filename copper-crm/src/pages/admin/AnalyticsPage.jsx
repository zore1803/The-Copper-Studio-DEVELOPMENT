    ...
    <Card>
      <div className="flex items-center justify-between border-b border-[#E5E7EB] px-5 py-4">
        ...
        <select 
          ...
          className="h-8 rounded-lg border border-[#E5E7EB] bg-[#FFFFFF] px-2 flex items-center text-xs font-bold text-[#6B7280] outline-none cursor-pointer hover:bg-[#E5E7EB]"
        >
              ...
              {users.map((u, i) => (
                <div key={u._id || u.id || i} onClick={() => navigate(`/admin/companies/${u._id || u.id}`)} className="cursor-pointer p-3 border border-[#E5E7EB] rounded-lg hover:border-[#C55418] hover:shadow-sm bg-[#ffffff] transition-all">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-[10px] font-bold text-[#6B7280] bg-[#FFFFFF] px-2 py-1 rounded">CLIENT</span>
                  </div>
                </div>
              ))}
            </div>
...
function KpiDrillDownPanel({ kpi, data, onClose }) {
  ...
  return (
    <SidePanel title={`${kpi} Details`} subtitle="Formula, result, and source records." onClose={onClose}>
      <div className="min-h-full space-y-4 bg-[#FFFFFF] p-5">
      ...
      action={
        <div className="flex flex-wrap items-center justify-end gap-2">
          ...
          <div className="flex items-center gap-2 rounded-xl border border-[#E5E7EB] bg-[#ffffff] p-1">
            <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="h-8 rounded-lg bg-transparent px-2 text-xs font-bold text-[#6B7280] outline-none hover:bg-[#E5E7EB] focus:bg-[#FFFFFF]">
            ...
            {filterType !== "All Time" && filterType !== "Custom Range" && (
              <select value={filterYear} onChange={(e) => setFilterYear(Number(e.target.value))} className="h-8 rounded-lg bg-transparent px-2 text-xs font-bold text-[#6B7280] outline-none border-l border-[#E5E7EB] hover:bg-[#E5E7EB] focus:bg-[#FFFFFF]">
            ...
            {filterType === "Monthly" && (
              <select value={filterMonth} onChange={(e) => setFilterMonth(Number(e.target.value))} className="h-8 rounded-lg bg-transparent px-2 text-xs font-bold text-[#6B7280] outline-none border-l border-[#E5E7EB] hover:bg-[#E5E7EB] focus:bg-[#FFFFFF]">
            ...
            {filterType === "Bi-Monthly" && (
              <select value={filterBiMonth} onChange={(e) => setFilterBiMonth(Number(e.target.value))} className="h-8 rounded-lg bg-transparent px-2 text-xs font-bold text-[#6B7280] outline-none border-l border-[#E5E7EB] hover:bg-[#E5E7EB] focus:bg-[#FFFFFF]">
                {["Jan-Feb", "Mar-Apr", "May-Jun", "Jul-Aug", "Sep-Oct", "Nov-Dec"].map((m, i) => <option key={i} value={i}>{m}</option>)}
              </select>
            )}

            {filterType === "Quarterly" && (
              <select value={filterQuarter} onChange={(e) => setFilterQuarter(Number(e.target.value))} className="h-8 rounded-lg bg-transparent px-2 text-xs font-bold text-[#6B7280] outline-none border-l border-[#E5E7EB] hover:bg-[#E5E7EB] focus:bg-[#FFFFFF]">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-[#E5E7EB] px-5 py-4 gap-3">
                ...
                <div className="flex gap-2">
                  {["All", "Revenue", "Users", "Projects"].map(m => (
                    <button key={m} onClick={() => setMetricFilter(m)} className={`px-2 py-1 text-[11px] font-bold rounded transition-colors ${metricFilter === m ? 'bg-[#C55418] text-white' : 'bg-[#FFFFFF] text-[#6B7280] hover:bg-gray-200'}`}>
                      {m}
                    </button>
                  ))}
                </div>
            ...
            <Card>
              ...
              <div className="grid grid-cols-3 gap-2 px-4 pb-4">
                {(data.statusData.length ? data.statusData : [{ name: "No data", value: 0, color: "#e5e7eb" }]).map((item) => (
                  <div key={item.name} className="rounded-xl bg-[#FFFFFF] p-3">
              ...
              <div>
                ...
                <div className="mt-4 space-y-3">
                  {(data.packageRevenue.length ? data.packageRevenue : [{ name: "No packages yet", revenue: 0, count: 0 }]).map((item) => (
                    <div 
                      key={item.name} 
                      className={`rounded-xl border bg-[#FFFFFF] p-3 transition-all ${item.count > 0 ? "cursor-pointer hover:border-[#C55418] hover:shadow-sm" : "border-[#E5E7EB]"}`}
      ...
      {selectedPackage && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/40 p-4 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="flex max-h-[85vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-[#ffffff] shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="flex-1 overflow-y-auto p-5 bg-[#FFFFFF]/50">
...
function PriorityProjectsTable({ projects, page, setPage, search, setSearch, navigate }) {
  ...
  const getBadgeColor = (level) => {
    switch (level) {
      ...
      case "Completed": return "bg-[#FFFFFF] text-[#1A1A1A] border-[#E5E7EB]";
      default: return "bg-[#FFFFFF] text-[#1A1A1A] border-[#E5E7EB]";
    }
  };
          ...
          <tbody className="divide-y divide-gray-100 bg-[#ffffff]">
            {displayed.length > 0 ? displayed.map(p => (
              ...
              >
                ...
                <td className="px-5 py-4">
                  <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium bg-[#FFFFFF] text-[#1A1A1A]">
...
function RecentPaymentsTable({ payments, page, setPage, search, setSearch, navigate }) {
  ...
  const getStatusColor = (status) => {
    ...
    if (s.includes("refund")) return "bg-[#FFFFFF] text-[#1A1A1A] border-[#E5E7EB]";
    return "bg-[#FFFFFF] text-[#1A1A1A] border-[#E5E7EB]";
  };
