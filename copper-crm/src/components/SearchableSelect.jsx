          ...
          <div className="absolute z-50 mt-1 max-h-56 w-full overflow-y-auto rounded-lg border border-[#e5e7eb] bg-white shadow-lg">
            {allowCustom && query.trim() && !normalized.some((o) => o.label.toLowerCase() === query.trim().toLowerCase()) && (
              <button
                ...
                className="block w-full truncate border-b border-[#FFFFFF] px-3 py-2 text-left text-sm text-[#5A1A14] hover:bg-[#E5E7EB]"
              >
                Use "{query.trim()}"
              </button>
            )}
              ...
              filtered.map((option) => (
                <button
                  ...
                  className={`block w-full truncate px-3 py-2 text-left text-sm hover:bg-[#E5E7EB] ${
                    String(option.value) === String(value || "") ? "bg-[#FFFFFF] font-semibold text-[#5A1A14]" : "text-[#1A1A1A]"
                  }`}
                >
                  {option.label}
                </button>
