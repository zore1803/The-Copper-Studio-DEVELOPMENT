...
export default function ProjectCard({ project, showClient = true }) {
  ...
  return (
    <Link
      ...
      className="block rounded-xl border border-[#e5e7eb] bg-white p-5 transition-all hover:border-[#C57E5B] hover:shadow-sm"
    >
      <div className="mb-3 flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="truncate text-sm font-bold text-[#111827]">{project.name || "Untitled project"}</p>
          {showClient && <p className="mt-0.5 text-xs text-[#525866]">{project.client}</p>}
        </div>
        <Badge color={statusColor[status] || "gray"}>{status}</Badge>
      </div>

      <div className="mb-4">
        <div className="mb-2 flex justify-between text-[11px] text-[#525866]">
          <span>Progress</span>
          <span className="font-bold text-[#0E121B]">{progress}%</span>
        </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex -space-x-1.5">
          {team.slice(0, 4).map((member, index) => (
            <div key={index} className="rounded-full ring-2 ring-[#FFFFFF]">
              <Avatar name={member} size="sm" />
            </div>
          ))}
          {!team.length && <span className="text-[11px] text-[#9ca3af]">No team assigned</span>}
        </div>
        <div className="flex items-center gap-1 text-[11px] text-[#525866]">
          <Calendar size={11} />
