...
export default function ProjectCard({ project, showClient = true }) {
  ...
  return (
    <Link
      ...
      className="group block rounded-xl border border-[#ead9d0] bg-white p-5 transition-all hover:border-[#D2743F] hover:shadow-md hover:shadow-[#C55418]/5"
    >
      <div className="mb-3 flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="truncate text-sm font-bold text-[#2b211c] group-hover:text-[#C55418]">{project.name || "Untitled project"}</p>
      ...
      <div className="mb-4">
        ...
        <div className="h-2 overflow-hidden rounded-full bg-[#f1e7e1]">
          <div className={`h-full rounded-full ${isDone ? "bg-emerald-500" : "bg-[#C55418]"}`} style={{ width: `${Math.min(100, progress)}%` }} />
        </div>
      </div>
        ...
        <div className="flex items-center">
          {team.length ? (
            <div className="flex -space-x-1.5">
              ...
              {team.length > 4 && (
                <div className="flex h-7 w-7 items-center justify-center rounded-full bg-[#f1e7e1] text-[10px] font-bold text-[#C55418] ring-2 ring-white">+{team.length - 4}</div>
              )}
            </div>
          ) : (
