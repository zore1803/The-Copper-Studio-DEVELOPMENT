  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <div className="rounded-lg border border-[#e5e7eb] bg-[#FFFFFF] p-3 space-y-3">
          <p className="text-xs font-bold uppercase tracking-wide text-[#6B7280]">Destination folder</p>
          {company ? (
            <div className="rounded-lg border border-[#e5e7eb] bg-white px-3 py-2 text-sm">
              <span className="text-[#6b7280]">Company:</span> <span className="font-semibold text-[#1A1A1A]">{company.name}</span>
            </div>
          ) : (
            <label className="block">
              ...
              <select
                ...
                className="mt-1.5 w-full rounded-lg border border-[#e5e7eb] bg-white px-3 py-2 text-sm outline-none focus:border-[#C55418] focus:ring-2 focus:ring-[#C55418]/20"
              >
          ...
          <label className="block">
            <span className="text-xs font-semibold text-[#1A1A1A]">Project folder</span>
            <select
              value={form.projectId}
              onChange={(e) => set("projectId")(e.target.value)}
              disabled={!form.companyId}
              className="mt-1.5 w-full rounded-lg border border-[#e5e7eb] bg-white px-3 py-2 text-sm outline-none focus:border-[#C55418] focus:ring-2 focus:ring-[#C55418]/20 disabled:bg-[#FFFFFF] disabled:text-[#6B7280]"
            >
        ...
        {/* File */}
        <label className="block">
          <span className="text-xs font-semibold text-[#1A1A1A]">File *</span>
          <div className="mt-1.5 rounded-lg border border-dashed border-[#6B7280] bg-[#FFFFFF] px-3 py-3">
            <div className="flex items-center gap-3">
              ...
              <label
                ...
                className={`rounded-lg bg-[#C55418] px-3 py-1.5 text-xs font-semibold text-white hover:bg-[#6B7280] ${reading || saving ? "pointer-events-none opacity-60" : "cursor-pointer"}`}
              >
                Browse…
              </label>
            ...
            {reading && (
              <div className="mt-2.5">
                <div className="mb-1 flex justify-between text-[10px] font-bold uppercase tracking-wide text-[#6B7280]">
                  <span>Reading file</span><span>{readPct}%</span>
                </div>
                <div className="h-1.5 overflow-hidden rounded-full bg-[#E5E7EB]">
                  <div className="h-full rounded-full bg-[#C55418] transition-all" style={{ width: `${readPct}%` }} />
                </div>
              </div>
            )}
            {saving && (
              <div className="mt-2.5">
                <div className="mb-1 flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wide text-[#C55418]">
                  <Loader2 size={11} className="animate-spin" /> Uploading to server…
                </div>
                <div className="h-1.5 overflow-hidden rounded-full bg-[#E5E7EB]">
                  <div className="upload-indeterminate h-full w-1/3 rounded-full bg-[#C55418]" />
                </div>
              </div>
            )}
          </div>
        ...
        <label className="block">
          ...
          <select value={form.fileType} onChange={(e) => set("fileType")(e.target.value)} className="mt-1.5 w-full rounded-lg border border-[#e5e7eb] px-3 py-2 text-sm outline-none focus:border-[#C55418] focus:ring-2 focus:ring-[#C55418]/20">
            {FILE_TYPES.map((type) => <option key={type}>{type}</option>)}
          </select>
        </label>

        <label className="block">
          <span className="text-xs font-semibold text-[#1A1A1A]">…or paste a file URL</span>
          <input
            value={fileReady ? "" : form.fileUrl}
            onChange={(e) => set("fileUrl")(e.target.value)}
            disabled={fileReady}
            className="mt-1.5 w-full rounded-lg border border-[#e5e7eb] px-3 py-2 text-sm outline-none focus:border-[#C55418] focus:ring-2 focus:ring-[#C55418]/20 disabled:bg-[#FFFFFF] disabled:text-[#6B7280]"
          />
