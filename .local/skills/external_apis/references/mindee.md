...
Use `externalApi__mindee` in `code_execution`.
...
```javascript
const result = await externalApi__mindee({
  path: '/v2/products/:product(extraction|ocr|classification|crop|split)/enqueue',
  method: 'POST',
  query: {},
})
